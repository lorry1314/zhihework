...
import com.zorgoom.zhihework.adapter.MyHouseLeaseAdapter;
 ...
 *
 */
public class MyHouseLeaseList extends MBaseActivity implements View.OnClickListener, HttpListener {

	private ListView message_listView1;
	private C2BHttpRequest c2BHttpRequest;
	private MyHouseLeaseList mContext;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.my_house_lease_layout);
		mContext = this;
		initView();
	...
	private void initData() {
		...
		c2BHttpRequest.getHttpResponse(
				Http.GETMYLEASEHOUSE + "USERID=" + userId + "&FKEY=" + key + "&TIMESTAMP=" + timestamp, 1);
	}
		...
		if (result != null) {
			...
			if (null != rsPropertypaymentListResultVO) {
				if ("101".equals(rsPropertypaymentListResultVO.getCode())) {
					...
					MyHouseLeaseAdapter myadapter = new MyHouseLeaseAdapter(this,
