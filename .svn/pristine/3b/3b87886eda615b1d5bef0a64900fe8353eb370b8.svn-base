...
import com.zorgoom.zhihework.adapter.AttenceDetailAdapter;
...
import com.zorgoom.zhihework.vo.RsAttenceDetail;
 ...
 *
 */
public class DaKaJiLuActivity extends MBaseActivity implements View.OnClickListener, HttpListener {
	...
	private DaKaJiLuActivity mContext;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.dakajilu_laout);
	...
	private void initData() {
		...
		c2BHttpRequest
				.getHttpResponse(Http.ATTENCEDETAIL + "USERID=" + userId + "&FKEY=" + key + "&TIMESTAMP=" + timestamp, 1);
	}
	...
	RsAttenceDetail rsPropertypaymentListResultVO;
	private AttenceDetailAdapter myadapter;
	...
	public void OnResponse(String result, int reqId) {
		...
		if (onResponseResult != null) {
			switch (reqId) {
			case 1:
				rsPropertypaymentListResultVO = gson.fromJson(onResponseResult, RsAttenceDetail.class);
				if ("101".equals(rsPropertypaymentListResultVO.getCode())) {
					...
					myadapter = new AttenceDetailAdapter(this, rsPropertypaymentListResultVO.getData());
