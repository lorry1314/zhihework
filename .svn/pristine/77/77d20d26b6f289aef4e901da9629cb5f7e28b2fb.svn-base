...
public class ChargeDetail_Main extends MBaseActivity implements View.OnClickListener, HttpListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.chargedetail_laout);
		initView();
	...
	private void initData() {
		...
		c2BHttpRequest
				.getHttpResponse(Http.CHARGEDETAIL + "USERID=" + userId + "&FKEY=" + key + "&TIMESTAMP=" + timestamp, 1);
	}
	...
	RsChargeDetail rsPropertypaymentListResultVO;
	private ChargeDetailAdapter myadapter;
	...
	public void OnResponse(String result, int reqId) {
		...
		if (onResponseResult != null) {
			switch (reqId) {
			case 1:
				rsPropertypaymentListResultVO = gson.fromJson(onResponseResult, RsChargeDetail.class);
				if ("101".equals(rsPropertypaymentListResultVO.getCode())) {
					...
					myadapter = new ChargeDetailAdapter(this, rsPropertypaymentListResultVO.getData());
