...
import com.zorgoom.zhihework.MainActivity;
import com.zorgoom.zhihework.R;
import com.zorgoom.zhihework.Start_Main;
...
public class UpdateService extends Service {
	...
	private Handler updateHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case DOWNLOAD_COMPLETE:
				...
				// "智慧书城商城", "下载完成，点击安装", updatePendingIntent);
				// updateNotificationManager.notify(0, updateNotification);

				updateNotificationManager.cancelAll();
				Intent intent2 = new Intent(Start_Main.DOWNLOADFILE);
				intent2.putExtra("updateFile", updateFile.getPath());
			...
			case DOWNLOAD_FAIL:
				...
				// updateNotification.setLatestEventInfo(UpdateService.this,
				// "腾讯QQ", "下载失败，sorry", updatePendingIntent);
				// updateNotificationManager.notify(0, updateNotification);
				break;
			default:
				stopSelf();
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (android.os.Environment.MEDIA_MOUNTED.equals(android.os.Environment.getExternalStorageState())) {
			// 组合下载地址
			updateDir = new File(Environment.getExternalStorageDirectory(), "/mnt/sdcard/bmowo/download/");
		} else {
			// files目录
		...
		updateNotification.setLatestEventInfo(this, "英伦管家", "0%", updatePendingIntent);
		// // 发出通知
		updateNotificationManager.notify(0, updateNotification);
