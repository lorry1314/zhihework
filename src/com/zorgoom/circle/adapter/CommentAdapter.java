...
public class CommentAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		try {
			if (convertView == null) {
				...
				convertView = View.inflate(mContext, R.layout.cf_im_social_item_comment, null);
			...
			subjectSpanText.setSpan(new NameClickable(new NameClickListener(subjectSpanText, "")), 0,
			...
			if (!TextUtils.isEmpty(toReplyName)) {
				...
				objectSpanText.setSpan(new NameClickable(new NameClickListener(objectSpanText, "")), 0,
						objectSpanText.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	class NameClickable extends ClickableSpan implements OnClickListener {
		...
		public void updateDrawState(TextPaint ds) {
			...
			int colorValue = mContext.getResources().getColor(R.color.color_8290AF);
			...
			if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_DOWN) {
				...
				if (link.length != 0) {
					...
					if (action == MotionEvent.ACTION_UP) {
						...
						// 点击选中效果
						((View) widget.getParent()).setBackgroundResource(R.color.adapter_item_phone_selector_color);
					} else {
						((View) widget.getParent()).setBackgroundResource(R.color.transparent);
					}
					Selection.removeSelection(buffer);
